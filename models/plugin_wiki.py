...
DEFAULT = {
    ...
    'authorize_attachments' : False # should attachment be restricted to the page?
    }
...
""" % dict(callback=callback,colnames=json(colnames),
    ...
    @staticmethod
    def hs_map(map_id, width = '100%', height = '500px', float = 'none'):
        width = require_style_attr (width)
        height = require_style_attr (height)
        return DIV (DIV (_class="hs_map", _map_id = str (map_id), _id = uuid4 ().hex, _style = "width: %s; height: %s;" % (width, height)),
                    A ("Add Map to My Panel", _href = "#", _class ="map_add", _map_id = str (map_id)), _style = "margin: 20px; float: %s;" % (float,))
...
class PluginWiki(object):
    ...
    def render_markmin(self,text,page_url=URL()):
        ...
        text = re.sub('\[\[(?P<t>[^\[\]]+) page:(?P<p>.+)\]\]','[[\g<t> %s/\g<p>/view]]' % page_url,text)
